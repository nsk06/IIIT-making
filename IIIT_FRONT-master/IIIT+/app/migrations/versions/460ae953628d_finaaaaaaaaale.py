...
def upgrade():
    ...
    sa.Column('organiser', sa.Integer(), nullable=True),
    ...
    sa.ForeignKeyConstraint(['organiser'], ['user.username'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_event_At_time'), 'event', ['At_time'], unique=False)
...
def downgrade():
    ...
    op.drop_index(op.f('ix_event_At_time'), table_name='event')
    op.drop_table('event')
