...
ad = "nonidh"
#print(app.config)
admin = Admin(app,name='Nsk06', template_mode='bootstrap3')
...
@login_required
def like(pos):
    x = Like.query.filter(Like.po == pos).filter(Like.liker == current_user.username).all()
    print(len(x))
    if len(x) >= 1:
        flash('already liked')
        return redirect(url_for('index'))
...
def GroupEvent(groupname):
    form = EventForm()
    if form.validate_on_submit():
        event = Event(ev=form.ev.data,At_time=form.At_time.data,location=form.location.data,organiser=current_user.username,gr = groupname,participants = 1)
